        ...
        <form method="POST" action="{{ route('backoffice.forbidden.update', $rule) }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea id="instruction" name="instruction" rows="4"
                    style="background-color:rgba(15,23,42,0.7);color:#e2e8f0" class="w-full rounded-2xl border border-white/10 bg-slate-900/70 px-4 py-3 text-sm text-white outline-none transition focus:border-cyan-400">{{ old('instruction', $rule->instruction) }}</textarea>
            </div>
