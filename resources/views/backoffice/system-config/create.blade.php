...
@section('title', 'New System Config Entry')
...
@section('content')
    ...
    <div style="display:flex;align-items:center;justify-content:space-between"
        ...
        <div>
            <h1 class="text-lg font-semibold sm:text-2xl">New System Config Entry</h1>
            <p class="text-xs text-slate-400">Add a global key / value configuration entry.</p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-700/70 bg-slate-900/85 p-4 sm:p-5">
        <form method="POST" action="{{ route('backoffice.system-config.store') }}" class="space-y-4">
            @csrf

            <div>
                <label class="bo-label" for="source_type">Source Type</label>
                <select id="source_type" name="source_type" onchange="scSyncSourceType()">
                    <option value="manual" {{ old('source_type', 'manual') === 'manual' ? 'selected' : '' }}>Manual</option>
                    <option value="datamodel_lookup" {{ old('source_type') === 'datamodel_lookup' ? 'selected' : '' }}>
                        DataModel Lookup</option>
                </select>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 2fr;gap:0.75rem;align-items:start">
                <div>
                    ...
                    <input id="key" type="text" name="key" value="{{ old('key') }}" required maxlength="191"
                ...
                <div id="manual-wrap">
                    ...
                    <textarea id="value" name="value" rows="3" placeholder="Config value...">{{ old('value') }}</textarea>
                </div>
            </div>
            ...
            <div id="dm-wrap" class="rounded-lg border border-white/10 bg-slate-900/50 p-3" style="display:none">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;align-items:start">
                    ...
                    <div>
                        ...
                        <input id="lookup_value" type="text" name="lookup_value" value="{{ old('lookup_value') }}"
...
@section('scripts')
    <script>
        ...
        function scPopulateFields() {
            const dmId = document.getElementById('data_model_id')?.value ?? '';
            const fields = scDataModels[String(dmId)]?.fields ?? [];
            scSetFieldOptions(document.getElementById('lookup_field'), fields, @json(old('lookup_field')));
            scSetFieldOptions(document.getElementById('result_field'), fields, @json(old('result_field')));
        }
        ...
        function scSyncSourceType() {
            ...
            if (t === 'datamodel_lookup') scPopulateFields();
        }
