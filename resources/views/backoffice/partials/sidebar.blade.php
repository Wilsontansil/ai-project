        ...
        <div class="bo-section" data-section="ai-agent">
            ...
            <div class="bo-section-items" style="margin-top:0.25rem;">
                ...
                @can('manage agent-rules')
                    <a href="{{ route('backoffice.agent-rules.index') }}" class="bo-nav-item group"
                        style="display:flex;align-items:center;gap:0.625rem;border-radius:0.5rem;padding:0.5rem 0.75rem;font-size:0.8125rem;text-decoration:none;transition:background 0.15s;{{ ($boActive ?? ($active ?? '')) === 'agent-rules' ? 'background:rgba(255,255,255,0.12);font-weight:600;color:#fff;' : 'color:rgba(255,255,255,0.7);' }}"
                        onmouseover="this.style.background='rgba(255,255,255,0.1)';this.style.color='#fff'"
                        onmouseout="this.style.background='{{ ($boActive ?? ($active ?? '')) === 'agent-rules' ? 'rgba(255,255,255,0.12)' : 'transparent' }}';this.style.color='{{ ($boActive ?? ($active ?? '')) === 'agent-rules' ? '#fff' : 'rgba(255,255,255,0.7)' }}'">
                        <svg style="width:18px;height:18px;flex-shrink:0;{{ ($boActive ?? ($active ?? '')) === 'agent-rules' ? 'color:#fff;' : 'color:rgba(255,255,255,0.45);' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5" width="18"
                            height="18">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
                        </svg>
                        <span class="bo-label">Agent Rules</span>
                        <span class="bo-tooltip">Agent Rules</span>
                    </a>
                @endcan
            ...
            <div class="bo-section-items" style="margin-top:0.25rem;">
                ...
                @can('manage settings')
                    <a href="{{ route('backoffice.system-config.index') }}" class="bo-nav-item group"
                        ...
                        onmouseover="this.style.background='rgba(255,255,255,0.1)';this.style.color='#fff'"
                        onmouseout="this.style.background='{{ ($boActive ?? ($active ?? '')) === 'system-config' ? 'rgba(255,255,255,0.12)' : 'transparent' }}';this.style.color='{{ ($boActive ?? ($active ?? '')) === 'system-config' ? '#fff' : 'rgba(255,255,255,0.7)' }}'">
                        <svg style="width:18px;height:18px;flex-shrink:0;{{ ($boActive ?? ($active ?? '')) === 'system-config' ? 'color:#fff;' : 'color:rgba(255,255,255,0.45);' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M4.5 12a7.5 7.5 0 0015 0m-15 0a7.5 7.5 0 1115 0m-15 0H3m16.5 0H21m-1.5 0H12m-8.457 3.077l1.41-.513m14.095-5.13l1.41-.513M5.106 17.785l1.15-.964m11.49-9.642l1.149-.964M7.501 19.795l.75-1.3m7.5-12.99l.75-1.3m-6.063 16.658l.26-1.477m2.605-14.772l.26-1.477m0 17.726l-.26-1.477M10.698 4.614l-.26-1.477M16.5 19.794l-.75-1.299m-7.5-12.99l-.75-1.3m6.063 16.658l-.26-1.477M7.702 4.614l.26-1.477" />
                        </svg>
                        <span class="bo-label">System Config</span>
                        <span class="bo-tooltip">System Config</span>
                    </a>
                @endcan
                ...
                {{-- Case Log --}}
                @can('view metrics')
                    <a href="{{ route('backoffice.technical.case-logs') }}" class="bo-nav-item group"
                        style="display:flex;align-items:center;gap:0.625rem;border-radius:0.5rem;padding:0.5rem 0.75rem;font-size:0.8125rem;text-decoration:none;transition:background 0.15s;{{ ($boActive ?? ($active ?? '')) === 'case-logs' ? 'background:rgba(255,255,255,0.12);font-weight:600;color:#fff;' : 'color:rgba(255,255,255,0.7);' }}"
                        onmouseover="this.style.background='rgba(255,255,255,0.1)';this.style.color='#fff'"
                        onmouseout="this.style.background='{{ ($boActive ?? ($active ?? '')) === 'case-logs' ? 'rgba(255,255,255,0.12)' : 'transparent' }}';this.style.color='{{ ($boActive ?? ($active ?? '')) === 'case-logs' ? '#fff' : 'rgba(255,255,255,0.7)' }}'">
                        <svg style="width:18px;height:18px;flex-shrink:0;{{ ($boActive ?? ($active ?? '')) === 'case-logs' ? 'color:#fff;' : 'color:rgba(255,255,255,0.45);' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5" width="18"
                            height="18">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M20.25 8.511c.884.284 1.5 1.128 1.5 2.097v4.286c0 1.136-.847 2.1-1.98 2.193-.34.027-.68.052-1.02.072v3.091l-3-3c-1.354 0-2.694-.055-4.02-.163a2.115 2.115 0 01-.825-.242m9.345-8.334a2.126 2.126 0 00-.476-.095 48.64 48.64 0 00-8.048 0c-1.131.094-1.976 1.057-1.976 2.192v4.286c0 .837.46 1.58 1.155 1.951m9.345-8.334V6.637c0-1.621-1.152-3.026-2.76-3.235A48.455 48.455 0 0011.25 3c-2.115 0-4.198.137-6.24.402-1.608.209-2.76 1.614-2.76 3.235v6.226c0 1.621 1.152 3.026 2.76 3.235.577.075 1.157.14 1.74.194V21l4.155-4.155" />
                        </svg>
                        <span class="bo-label">Case Log</span>
                        <span class="bo-tooltip">Case Log</span>
                    </a>
                @endcan
