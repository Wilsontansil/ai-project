...
@section('content')
        ...
        <form method="POST" action="{{ route('backoffice.tools.update', $tool) }}" class="space-y-5">
            ...
            <div class="grid gap-4 md:grid-cols-4">
                ...
                <div>
                    ...
                    <select id="type" name="type"
                        class="w-full rounded-2xl border border-white/10 bg-slate-900/70 px-4 py-3 text-sm text-white outline-none transition focus:border-cyan-400">
                        <option value="info" {{ old('type', $tool->type) === 'info' ? 'selected' : '' }}>
                            {{ __('backoffice.pages.tools.type_info') }}</option>
                        <option value="get" {{ old('type', $tool->type) === 'get' ? 'selected' : '' }}>
                            {{ __('backoffice.pages.tools.type_get') }}</option>
                        <option value="get_multiple" {{ old('type', $tool->type) === 'get_multiple' ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input id="keywords" type="text" name="keywords"
                    value="{{ old('keywords', $tool->keywords ? implode(', ', $tool->keywords) : '') }}" maxlength="500"
                    oninput="document.getElementById('kw-count').textContent=this.value.length"
                    class="w-full rounded-2xl border border-white/10 bg-slate-900/70 px-4 py-3 text-sm text-white outline-none transition focus:border-cyan-400" />
                <p class="mt-1 text-xs text-slate-500"><span
                        id="kw-count">{{ strlen(old('keywords', $tool->keywords ? implode(', ', $tool->keywords) : '') ?? '') }}</span>/500
                </p>
            </div>
            ...
            <div>
                ...
                <input id="description" type="text" name="description"
                    value="{{ old('description', $tool->description) }}" maxlength="500"
                    oninput="document.getElementById('desc-count').textContent=this.value.length"
                    class="w-full rounded-2xl border border-white/10 bg-slate-900/70 px-4 py-3 text-sm text-white outline-none transition focus:border-cyan-400" />
                <p class="mt-1 text-xs text-slate-500"><span
                        id="desc-count">{{ strlen(old('description', $tool->description) ?? '') }}</span>/500</p>
            </div>
            ...
            <div id="section-get" class="space-y-4" style="display:none">
                ...
                <div class="rounded-2xl border border-cyan-400/20 bg-cyan-500/5 p-4 space-y-4">
                    ...
                    <div id="query-config-body">
                        ...
                        {{-- Select fields --}}
                        <div>
                            <p class="mb-1 text-sm text-slate-200">Select Fields <span class="text-xs text-slate-400">(leave
                                    all
            ...
            <div id="section-update" class="space-y-4" style="display:none">
                <div>
                    <p class="mb-2 block text-sm text-slate-200">{{ __('backoffice.pages.tools.parameters') }}</p>
                    <p class="mb-2 text-xs text-slate-400">{{ __('backoffice.pages.tools.parameters_api_help') }}</p>
  ...
  "data": {}
                            ...
                            <p class="mb-2 text-xs text-slate-400">{{ __('backoffice.pages.tools.expected_data_help') }}
                            </p>
                            ...
                            <div class="hidden">
                                ...
                                <input type="hidden" name="endpoint_expected_message"
                                    value="{{ old('endpoint_expected_message', data_get($tool->endpoints, 'endpoint.expected_response.message', data_get($tool->endpoints, 'get.expected_response.message', 'Success'))) }}" />
                            </div>
                        </div>
                                ...
                                <pre id="error-response-preview" class="text-xs text-slate-300 whitespace-pre-wrap font-mono overflow-auto max-h-64">[
  {
    ...
    "data": {}
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500"><span
                        id="rules-count">{{ strlen(old('tool_rules', $tool->tool_rules) ?? '') }}</span>/1000</p>
            </div>
...
@section('scripts')
    <script>
        ...
        function buildConditionFieldOptions(selected) {
            const fields = getSelectedDataModelFields();
            let html = `<option value="${selected || ''}">${selected || '— type or select —'}</option>`;
            fields.forEach(f => {
                if (f !== selected) html += `<option value="${f}">${f}</option>`;
            });
            return html;
        }
        ...
        /* ── Form init ── */
        const existingQuerySelect = @json(data_get($tool->meta, 'query.select', []));
        const existingConditions = @json(data_get($tool->meta, 'query.conditions', []));
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (ep.error_responses && Array.isArray(ep.error_responses) && ep.error_responses.length > 0) {
                ep.error_responses.forEach(er => addErrorResponse(er.status || '', er.message || ''));
            } else {
            ...
            refreshParameterFieldOptions(); // also refreshes select list + condition datalists

            // Pre-populate query conditions
            existingConditions.forEach(cond => {
                addConditionRow(
                    ...
                    cond.arg || '',
                    cond.group !== undefined ? String(cond.group) : '',
                    !!cond.skip_if_empty,
                    !!cond.required
                );
            });
