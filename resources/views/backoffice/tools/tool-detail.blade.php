...
@section('title', $tool->display_name)

@section('content')
    <div class="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
        <h1 class="text-3xl font-semibold">{{ $tool->display_name }}</h1>
    ...
    <div class="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
        ...
        <form method="POST" action="{{ route('backoffice.tools.update', $tool->slug) }}" class="mt-6 space-y-4">
            @csrf
    ...
    <div class="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
        ...
        <p class="mt-1 text-sm text-slate-300">Recent {{ strtolower($tool->display_name) }} requests processed by AI agent.
        </p>
