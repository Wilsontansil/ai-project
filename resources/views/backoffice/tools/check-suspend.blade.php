...
@section('title', 'Check Suspend Tool')

@section('content')
    <div class="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
        <h1 class="text-3xl font-semibold">Check Suspend Tool</h1>
        <p class="mt-2 text-sm text-slate-300">Tool untuk cek status suspend akun player.</p>
    </div>
    ...
    <div class="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
        ...
        <p class="mt-1 text-xs text-slate-400">Tool key: checkSuspend</p>

        <form method="POST" action="{{ route('backoffice.tools.check-suspend.update') }}" class="mt-6 space-y-4">
            @csrf
    ...
    <div class="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
        ...
        <p class="mt-1 text-sm text-slate-300">Recent check suspend requests processed by AI agent.</p>
